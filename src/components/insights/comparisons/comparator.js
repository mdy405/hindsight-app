...
const alignDateRate = (account, target) => {
  ...
  if (targetRate > accountRate) {
    ...
    const startingIndex = target.findIndex(
      (day) =>
        daysBetween(account[0].date, day.date) >= 0 &&
        daysBetween(account[0].date, day.date) <= 7
    );
    ...
    if (alignedTarget[0].date !== alignedAccount[0].date) {
      const adjustAccount = alignedAccount.findIndex(
        (day) => day.date === alignedTarget[0].date
      );
      alignedAccount = alignedAccount.slice(
        adjustAccount,
        alignedAccount.length
      );
    }
...
const synchronizeData = (account, target) => {
  ...
  if (accountStart > targetStart) {
    const adjustTarget = targetData.findIndex(
      (day) => day.date === accountData[0].date
    );
    targetData = targetData.slice(adjustTarget, targetData.length);
  } else if (targetStart > accountStart) {
    const adjustAccount = accountData.findIndex(
      (day) => day.date === targetData[0].date
    );
  ...
  if (accountEnd > targetEnd) {
    const adjustAccount = accountData.findIndex(
      (day) => day.date === targetData[targetData.length - 1].date
    );
    accountData = accountData.slice(0, adjustAccount + 1);
  } else if (targetEnd > accountEnd) {
    const adjustTarget = targetData.findIndex(
      (day) => day.date === accountData[accountData.length - 1].date
    );
...
const determineDeposits = (data, index, depositRecords) => {
  const inProgressDeposits = depositRecords.filter((record) => {
    try {
      if (record.object === "deposit" && record.accepted_at !== null) {
        const daysDiff = daysBetween(
          record.accepted_at.split("T")[0],
          data[index].date
        );
...
export default function buildComparison(account, target) {
  ...
  // line up.
  const { accountData, targetData } = synchronizeData(
    account.data,
    target.data
  );
  ...
  const targetGain = (index) => {
    ...
    const percentGain =
      (targetData[index].adjusted_price -
        targetData[index - 1].adjusted_price) /
      targetData[index - 1].adjusted_price;
    return (
      percentGain * determineDeposits(accountData, index, account.deposits)
    );
  };

  let targetTotalGain = 0,
    accountTotalGain = 0;
  let startingPortfolioGain =
    accountData[0].value.amount - accountData[0].net_deposits.amount;

  return accountData.map((day, i) => {
    targetTotalGain += targetGain(i);
    accountTotalGain =
      day.value.amount - day.net_deposits.amount - startingPortfolioGain;
