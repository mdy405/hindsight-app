...
const Header = (props) => {
  ...
  const displayAccounts = Object.keys(props.location.state.performance).map(
    (account) => accountNames[account]
  );
...
const Insight = (props) =>
  React.cloneElement(supportedInsights[props.index], props);
...
const Insights = AuthenticatedView((props) => {
  ...
  const [account, setAccount] = useState(
    accountNames[Object.keys(data.performance)[0]]
  );
